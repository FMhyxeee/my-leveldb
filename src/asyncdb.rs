...
use crate::{error::StatusCode, Options, Result, Status, WriteBatch, DB};
...
/// A response sent from the database thread.
enum Response {
    OK,
    Error(Status),
    Value(Option<Vec<u8>>),
...
/// mechanism as well as channel types.
pub struct AsyncDB {
    jh: JoinHandle<()>,
...
impl AsyncDB {
    ...
    /// As snapshots returned by `AsyncDB::get_snapshot()` are sort-of "week references" to an
