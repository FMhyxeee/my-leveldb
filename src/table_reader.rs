...
//         let mut table = Table::new_raw(opt.clone(), wrap_buffer(src).into(), size).unwrap();
...
//         let mut table = Table::new_raw(Options::default(), wrap_buffer(src).into(), size).unwrap();
...
//         let mut table = Table::new(Options::default(), wrap_buffer(src).into(), size).unwrap();
