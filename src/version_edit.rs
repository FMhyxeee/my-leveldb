...
use crate::{
    ...
    types::{FileMetaData, FileNum, SequenceNumber},
};

#[derive(PartialEq, Debug, Clone)]
pub struct CompactionPointer {
    pub level: usize,
    // This key is in InternalKye format.
    pub key: Vec<u8>,
}
...
impl VersionEdit {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        if let Some(lognum) = self.log_number {
            let _ = buf.write_varint(EditTag::LogNumber as u32).is_ok();
            let _ = buf.write_varint(lognum).is_ok();
        }

        if let Some(plognum) = self.prev_log_number {
            let _ = buf.write_varint(EditTag::PrevLogNumber as u32).is_ok();
            let _ = buf.write_varint(plognum).is_ok();
        }

        if let Some(nfn) = self.next_file_number {
            let _ = buf.write_varint(EditTag::NextFileNumber as u32).is_ok();
            let _ = buf.write_varint(nfn).is_ok();
        }

        if let Some(ls) = self.last_seq {
            let _ = buf.write_varint(EditTag::LastSequence as u32).is_ok();
            let _ = buf.write_varint(ls).is_ok();
        }

        for cptr in self.compaction_ptrs.iter() {
            let _ = buf.write_varint(EditTag::CompactPointer as u32).is_ok();
            let _ = buf.write_varint(cptr.level).is_ok();
            let _ = buf.write_varint(cptr.key.len()).is_ok();
            let _ = buf.write(cptr.key.as_ref()).is_ok();
        }

        for df in self.deleted.iter() {
            let _ = buf.write_varint(EditTag::DeletedFile as u32).is_ok();
            let _ = buf.write_varint(df.0).is_ok();
            let _ = buf.write_varint(df.1).is_ok();
        }

        for nf in self.new_files.iter() {
            let _ = buf.write_varint(EditTag::NewFile as u32).is_ok();
            let _ = buf.write_varint(nf.0).is_ok();
            let _ = buf.write_varint(nf.1.num).is_ok();
            let _ = buf.write_varint(nf.1.size).is_ok();

            let _ = buf.write_varint(nf.1.smallest.len()).is_ok();
            let _ = buf.write(nf.1.smallest.as_ref()).is_ok();
            let _ = buf.write_varint(nf.1.largest.len()).is_ok();
            let _ = buf.write(nf.1.largest.as_ref()).is_ok();
        }
    ...
    pub fn decode_from(src: &[u8]) -> Result<VersionEdit> {
        ...
        while let Ok(tag) = reader.read_varint::<u32>() {
            if let Some(tag) = tag_to_enum(tag) {
                match tag {
                    EditTag::Comparator => {
                        let buf = read_length_prefixed(&mut reader).unwrap();
