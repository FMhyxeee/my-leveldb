...
use crate::{
    ...
    types::{FileMetaData, SequenceNumber, Status},
};
...
fn read_length_prefixed<R: Read>(reader: &mut R) -> Result<Vec<u8>, Status> {
    if let Ok(klen) = reader.read_varint() {
        ...
        if let Ok(l) = reader.read(&mut keybuf) {
            ...
            Err(Status::IOError("Couldn't read key".to_string()))
        }
    } else {
        Err(Status::IOError("Couldn't read key length".to_string()))
    }
}
...
impl VersionEdit {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        if let Some(plognum) = self.prev_log_number {
            ...
            buf.write_varint(plognum).unwrap();
        }
                ...
                match tag {
                    ...
                    EditTag::NextFileNumber => {
                        if let Ok(nfn) = reader.read_varint() {
                            ...
                            return Err(Status::IOError(
                                "Couldn't read next_file_number".to_string(),
                            ));
                        }
                    }
                    ...
                    EditTag::PrevLogNumber => {
                        if let Ok(pln) = reader.read_varint() {
                            ...
                            return Err(Status::IOError(
                                "Couldn't read prev_log_number".to_string(),
                            ));
                        }
                    }
                }
