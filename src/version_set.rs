...
impl VersionSet {
    ...
    /// live_files return the files that are currently active.
    ...
    /// write snapshot writes the current versions, with all files, to the manifest.
    ...
    /// log_and_apply merges the given edit with the current state and generates a new version.
    /// It writes the VersionEdit to the manifest.
    ...
    /// returns true, the a manifest needs to be written eventually (usning log_and_apply()).
...
impl Builder {
    ...
    // maybe_add_file adds a file f at level to version v, if it's not already marked
...
mod tests {
    ...
    fn example_files() -> Vec<FileMetaHandle> {
        let f1 = FileMetaData {
            num: 1,
            size: 10,
            smallest: "f".as_bytes().to_vec(),
            largest: "g".as_bytes().to_vec(),
            ..Default::default()
        };
        ...
        let f2 = FileMetaData {
            num: 2,
            size: 20,
            smallest: "e".as_bytes().to_vec(),
            largest: "f".as_bytes().to_vec(),
            ..Default::default()
        };
        ...
        let f3 = FileMetaData {
            num: 3,
            size: 30,
            smallest: "a".as_bytes().to_vec(),
            largest: "b".as_bytes().to_vec(),
            ..Default::default()
        };
        ...
        let f4 = FileMetaData {
            num: 4,
            size: 40,
            smallest: "q".as_bytes().to_vec(),
            largest: "z".as_bytes().to_vec(),
            ..Default::default()
        };
    ...
    fn test_version_set_builder() {
        ...
        let fmd = FileMetaData {
            num: 21,
            size: 123,
            smallest: LookupKey::new("klm".as_bytes(), 777)
                .internal_key()
                .to_vec(),
            largest: LookupKey::new("kop".as_bytes(), 700)
                .internal_key()
                .to_vec(),
            ..Default::default()
        };
        ...
        assert_eq!(
            &[120u8, 120, 120, 1, 123, 0, 0, 0, 0, 0, 0],
            ptrs[2].as_slice()
        );
    ...
    fn test_version_set_log_and_apply() {
        ...
        // Simulate compaction by adding a file.
        {
            ...
            let fmd = FileMetaData {
                num: 21,
                size: 123,
                smallest: LookupKey::new("abc".as_bytes(), 777)
                    .internal_key()
                    .to_vec(),
                largest: LookupKey::new("def".as_bytes(), 700)
                    .internal_key()
                    .to_vec(),
                ..Default::default()
            };
