...
use crate::{
    ...
    key_types::{parse_internal_key, InternalKey, LookupKey, UserKey},
    table_cache::TableCache,
    table_reader::TableIterator,
    types::{FileMetaData, LdbIterator, MAX_SEQUENCE_NUMBER},
};

const NUM_LEVELS: usize = 7;

/// FileMetaHandle is reference-counted FileMetaData object.
...
impl Version {
    fn new(cache: Rc<TableCache>, ucmp: Rc<Box<dyn Cmp>>) -> Version {
        Version {
            table_cache: cache,
            user_cmp: ucmp,
    ...
    fn update_stats(&mut self, stats: GetStats) -> bool {
        if let Some(mut file) = stats.file {
            {
                let f = Rc::get_mut(&mut file).unwrap();
                f.allowed_seeks -= 1;
            }
...
mod tests {
    ...
    use crate::{
        cmp::{DefaultCmp, InternalKeyCmp},
        key_types::LookupKey,
        types::{FileMetaData, MAX_SEQUENCE_NUMBER},
        version_set::{
