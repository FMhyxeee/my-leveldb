...
use crate::{
    cmp::Cmp,
    key_types::{parse_internal_key, truncate_to_userkey, LookupKey, ValueType},
    merging_iter::MergingIter,
    snapshot::Snapshot,
    types::{Direction, LdbIterator, Shared},
...
impl LdbIterator for DBIterator {
    ...
    fn prev(&mut self) -> bool {
        ...
        if self.dir == Direction::Forward {
            ...
            self.savedkey.clear();
            self.savedkey
                .extend_from_slice(parse_internal_key(&self.buf).2);
            loop {
                self.iter.prev();
                if !self.iter.valid() {
                    self.valid = false;
...
#[cfg(test)]
mod tests {
    use crate::{db_impl, types::current_key_val};
