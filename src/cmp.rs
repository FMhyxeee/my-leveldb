...
use crate::{
    key_types::{parse_internal_key, parse_memtable_key, parse_tag, LookupKey},
    types,
};
...
impl Cmp for InternalKeyCmp {
    ...
    fn find_shortest_sep(&self, a: &[u8], b: &[u8]) -> Vec<u8> {
        let (_, seqa, keya) = parse_internal_key(a);
        let (_, _, keyb) = parse_internal_key(b);
    ...
    fn find_short_succ(&self, a: &[u8]) -> Vec<u8> {
        let (_, seq, key) = parse_internal_key(a);
