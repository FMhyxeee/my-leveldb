...
impl<'a, 'b: 'a, C: 'b + Comparator> MergingIter<'a, 'b, C> {
    ...
    /// entry after (Fwd) or one entry before (Rrvs) the current entry.
...
mod tests {
    use super::*;
    use crate::{skipmap::tests::make_skipmap, test_util::TestLdbIter, types::StandardComparator};
    ...
    fn test_merging_one() {
        ...
        let miter = MergingIter::new(vec![&mut iter], StandardComparator);
