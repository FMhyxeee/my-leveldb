...
impl FilterPolicy for NoFilterPolicy {
    ...
    fn create_filter(&self, _: &[&[u8]]) -> Vec<u8> {
        vec![]
    }
...
/// A filter policy wrapping another policy; extracting the user key from internal key for all
/// operation.
/// A user Key is u8*.
